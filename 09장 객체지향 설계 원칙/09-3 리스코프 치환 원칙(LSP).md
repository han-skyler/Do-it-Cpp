...
- 리스코프 치환 원칙 이해하기
- is-a 관계와 리스코프 치환 원칙 연결해서 이해하기
</aside>

<br>

리스코프 치환 원칙(Liskov Substitution Principle)은 **‘하위 클래스는 상위 클래스를 대체할 수 있어야 한다’**는 의미이다. 이 원칙은 다형성의 동작 원리를 설명한다.

자식 클래스가 부모 클래스를 치환한다는 것은 부모 클래스의 역할을 자식 클래스가 수행할 수 있다는 이야기이다. ‘is-a’ 관계로 정의된 클래스는 리스코프 치환 원칙에 따르는 클래스이며, 이는 다음의 두 가지를 의미한다.
...
### 지형 클래스에 리스코프 치환 원칙 적용

지형이 가지는 특징을 최상위 부모 클래스에 정의하고 몬스터, 플레이어와 상호 작용할 수 있는 가상 함수를 만들어 자식 클래스들이 상속 받도록 한다.
...
몬스터들이 배치될 수 있는 지형은 모두 세 종류이다. 숲, 사이버 공간 그리고 도심이다. 세 가지 공간 모두 terrain 클래스를 상속 받는다. 주목할 코드는 `monster_factory::create_monster` 함수이다. 매개변수로 입력 받은 terrain 클래스의 포인터가 가리키는 실제 객체는 forest_terrain, cyber_terrain, urban_terrain 가운데 하나이다. 함수에서는 세 클래스 모두 terrain 클래스 객체처럼 동작한다. 이 부분이 리스코프 치환 원칙을 따르고 있다.
