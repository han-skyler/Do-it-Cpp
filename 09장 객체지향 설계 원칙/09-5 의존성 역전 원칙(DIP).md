...
의존성 역전 원칙(Dependency Inversion Principle)은 **‘상위 수준 모듈은 하위 수준의 모듈에 의존해서는 안되며 ‘상위·하위 수준 모두 추상 레이어(인터페이스)에 의존해야 한다.’**로 정리할 수 있다. 의존성 역전 원칙은 ‘개방·폐쇄 원칙’을 적용하기 위해 사용된다.
...
코드를 보면 플레이어 클래스가 구상 클래스를 통해 무기와 탈 것에 직접 의존하는 것이 아니라, 인터페이스를 통해서 의존하는 것을 알 수 있다.

이 코드에서는 자동차와 기관총을 사용하지만, 말과 화살을 사용하려면 set_weapon(), set_riding_object() 함수의 객체만 변경해주면 된다. 그리고 새로운 무기와 탈 것을 만들 때는 IWeapon, IRiding_object 인터페이스를 상속받아 구현하면 된다. 즉, 새로운 무기 클래스가 개발되더라도 플레이어 클래스는 수정할 필요가 없다. 개방·폐쇄 원칙에서 배웠던 수정에는 닫히고, 확장에는 열린 상태가 되었다.
